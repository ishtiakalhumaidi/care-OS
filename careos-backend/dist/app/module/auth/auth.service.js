...
const inviteUser = async (payload, inviterRole, inviterBranchId) => {
    ...
    if (existingInvite) {
        throw new AppError(status.CONFLICT, "An invitation is already pending for this email");
    }
...
const acceptInvite = async (payload) => {
    ...
    if (invitation.status === "EXPIRED" || invitation.expiresAt < new Date()) {
        ...
        throw new AppError(status.GONE, "This invitation has expired. Ask your admin to resend it.");
    }
    const isUserExist = await prisma.user.findUnique({
