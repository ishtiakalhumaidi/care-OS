...
interface UserSearchSelectProps {
  role: "GUARDIAN" | "TEACHER" | "CENTER_ADMIN";
  branchId?: string;
...
export default function UserSearchSelect({
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["users", "search", role, branchId, debouncedTerm],
    queryFn: () =>
      getUsers(
        `role=${role}${branchId ? `&branchId=${branchId}` : ""}&searchTerm=${encodeURIComponent(debouncedTerm)}&limit=10`,
      ),
